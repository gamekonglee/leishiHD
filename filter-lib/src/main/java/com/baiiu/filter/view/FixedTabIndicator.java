...
public class FixedTabIndicator extends LinearLayout {
    ...
    public int mCurrentIndicatorPosition;// 上一个指针选中条目
    public int mLastIndicatorPosition;// 上一个指针选中条目
    ...
    public void setTitles(MenuAdapter menuAdapter,boolean isFull) {
        if (menuAdapter == null) {
            return;
        }
        this.removeAllViews();
        ...
        mTabCount = menuAdapter.getMenuCount();
        for (int pos = 0; pos < mTabCount; ++pos) {
            addView(generateTextView02(menuAdapter.getMenuTitle(pos), pos, isFull));
        }
        postInvalidate();
    }
...
//    public static Boolean isFull=false;
     ...
     */
    private View generateTextView02(String title, int pos,boolean isFull) {
        // 子空间TextView
    ...
    private View generateTextView(String title, int pos) {
        ...
        rl.setId(pos);
        LayoutParams params=null;
        params = new LayoutParams(getScreenWidth((Activity) context)/2, -1);
//        params.weight = 1;
